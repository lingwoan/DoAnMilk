<?php
    ...
    if (!$username || !$maus || !$password || !$email || !$fullname || !$birthday || !$sex)
    {
    ...
    if (mysqli_num_rows($db->query("SELECT username FROM member WHERE username='$username'")) > 0){
    ...
    if (mysqli_num_rows($db->query("SELECT email FROM member WHERE email='$email'")) > 0)
    {
